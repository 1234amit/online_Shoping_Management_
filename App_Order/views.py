...
def remove_from_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        messages.info(request, 'You have no any active order')
        return redirect('App_Shop:home')
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            if order_item.quantity >= 1:
                ...
                order_item.add()
                messages.warning(request, f"{item.name} item has been added in your cart")
                return redirect("App_Order:cart")
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            if order_item.quantity > 1:
                ...
                order.orderitems.remove(order_item)
                order_item.delete()
                messages.warning(request, f"{item.name} item has been removed in your cart")
                return redirect("App_Order:cart")
